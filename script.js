...
const STORAGE_KEYS = {
    ...
    previewMode: "obsidian-preview-mode"
};
...
document.addEventListener("DOMContentLoaded", () => {
    bindUI();
    applySavedTheme();
    applySavedPreviewMode();
    syncDeviceStatusTime();
    runLoader();
...
window.setInterval(syncDeviceStatusTime, 60000);
...
function bindUI() {
    ...
    $("preview-desktop")?.addEventListener("click", () => setPreviewMode("desktop"));
    $("preview-mobile")?.addEventListener("click", () => setPreviewMode("mobile"));
    ...
    $("print-chat-action")?.addEventListener("click", printChatAsPdf);
    ...
    $("display-name")?.addEventListener("input", syncSelectedDisplayName);
...
function syncDeviceStatusTime() {
    const timeNode = $("device-statusbar-time");
    if (!timeNode) return;
    const now = new Date();
...
function applySavedPreviewMode() {
    const saved = localStorage.getItem(STORAGE_KEYS.previewMode);
    setPreviewMode(saved === "mobile" ? "mobile" : "desktop", false);
}

function setPreviewMode(mode, persist = true) {
    state.previewMode = mode === "mobile" ? "mobile" : "desktop";
    $("preview-desktop")?.classList.toggle("active", state.previewMode === "desktop");
    $("preview-mobile")?.classList.toggle("active", state.previewMode === "mobile");
    $("preview-stage")?.classList.toggle("is-mobile-preview", state.previewMode === "mobile");
    $("device-shell")?.setAttribute("data-preview-mode", state.previewMode);
    document.body.classList.toggle("mobile-preview-active", state.previewMode === "mobile");
...
function setupDropTarget() {
    ...
    dropTarget.addEventListener("drop", async (event) => {
        ...
        await preloadDisplayNameOptions(files[0]);
    });
}
...
async function handleFileInputChange(event) {
    ...
    if (file) {
        ...
        await preloadDisplayNameOptions(file);
    }
}
...
function syncSelectedDisplayName(event) {
...
async function initViewer() {
    ...
    try {
        ...
        if (!hasMatchedOwnSender()) {
            const suggestions = Object.keys(state.senderStats).slice(0, 3).join(" / ");
            showToast(suggestions ? `Display name not found. Try: ${suggestions}` : "Display name not found in chat export");
            return;
        }
...
async function preloadDisplayNameOptions(file) {
    ...
    try {
        ...
        if (nameInput && !nameInput.value.trim() && senders.length) {
            nameInput.value = senders[0];
            state.myName = senders[0];
        }
    } catch (error) {
        console.error(error);
...
async function loadChatTextPreview(file) {
    if (!file.name.toLowerCase().endsWith(".zip")) {
...
function extractSenderCandidates(text) {
...
function renderChatList() {
    ...
    const slice = buildRenderableEntries(state.filteredMessages.slice(state.renderRange.start, state.renderRange.end));
    const participantNames = new Set(
        state.messages
            .filter((entry) => entry.type === "msg")
            .map((entry) => entry.sender)
    );
    const isDirectChat = participantNames.size <= 2;
    let lastSender = null;
    let html = "";
    ...
    slice.forEach((item) => {
        ...
        const rowClass = `msg-row ${item.isMe ? "sent" : "received"} ${isFirst ? "tail" : ""} ${tailClass} ${isDirectChat ? "direct-chat" : ""}`.trim();
        const senderHtml = !isDirectChat && !item.isMe && isFirst
...
function buildRenderableEntries(entries) {
    const grouped = [];
    ...
    entries.forEach((entry) => {
        ...
        if (canMergeMediaCluster(previous, entry)) {
            previous.mediaItems.push(...entry.mediaItems);
            previous.time = entry.time;
            previous.id = `${previous.id}__${entry.id}`;
            return;
        }
...
function canMergeMediaCluster(previous, current) {
    if (!previous || previous.type !== "msg" || current.type !== "msg") return false;
    if (previous.sender !== current.sender || previous.isMe !== current.isMe) return false;
    if (!isMediaOnlyMessage(previous) || !isMediaOnlyMessage(current)) return false;
    return true;
}

function isMediaOnlyMessage(message) {
    return message.type === "msg" && !message.text.trim() && Array.isArray(message.mediaItems) && message.mediaItems.length > 0;
}

function renderMediaStack(mediaItems) {
    const galleryItems = mediaItems.filter((item) => item.status === "available" && (item.kind === "image" || item.kind === "video"));
    const otherItems = mediaItems.filter((item) => !galleryItems.includes(item));
    const galleryIds = galleryItems.map((item) => item.id);

    return `
        <div class="msg-media-stack ${galleryItems.length > 1 ? "has-gallery" : ""}">
            ${galleryItems.length ? renderMediaGallery(galleryItems, galleryIds) : ""}
            ${otherItems.map((item) => renderMediaItem(item, galleryIds)).join("")}
        </div>
    `;
}

function renderMediaGallery(items, groupIds = []) {
    if (items.length === 1) {
        return renderMediaItem(items[0], groupIds);
    }

    const previewItems = items.slice(0, 4);
    const hiddenCount = items.length - previewItems.length;
    const galleryClass = `media-gallery media-gallery-${Math.min(previewItems.length, 4)}`;

    return `
        <div class="${galleryClass}">
            ${previewItems.map((item, index) => renderGalleryTile(item, index, hiddenCount, groupIds)).join("")}
        </div>
    `;
}

function renderGalleryTile(item, index, hiddenCount, groupIds = []) {
    ...
    const groupAttr = groupIds.length ? `data-media-group="${escapeAttribute(groupIds.join(","))}"` : "";
    const overlay = hiddenCount > 0 && index === 3
        ? `<span class="media-gallery-more">+${hiddenCount}</span>`
        : "";

    if (item.kind === "video") {
        const videoSrc = isLoaded ? `src="${escapeAttribute(item.url)}" class="loaded"` : `data-src="${escapeAttribute(item.url)}"`;
        return `
            <button class="media-button media-gallery-tile media-video" type="button" data-open-media="${item.id}" ${groupAttr}>
                <div class="media-gallery-video-frame">
                    <video preload="none" ${videoSrc} data-media-id="${item.id}" muted playsinline></video>
                    <span class="media-gallery-play"><i class="ph-fill ph-play"></i></span>
                    ${overlay}
                </div>
            </button>
        `;
    ...
    return `
        <button class="media-button media-gallery-tile media-image" type="button" data-open-media="${item.id}" ${groupAttr}>
...
function renderMediaItem(item, groupIds = []) {
    ...
    if (item.kind === "image") {
        return `
            <button class="media-button media-image" type="button" data-open-media="${item.id}" ${groupAttr}>
                <img ${srcAttr} data-media-id="${item.id}" alt="${escapeAttribute(item.name)}" loading="lazy" decoding="async">
            </button>
        `;
    }
    ...
    if (item.kind === "sticker") {
        return `
            <button class="media-button media-sticker" type="button" data-open-media="${item.id}" ${groupAttr}>
                <img ${srcAttr} data-media-id="${item.id}" alt="${escapeAttribute(item.name)}" loading="lazy" decoding="async">
            </button>
        `;
    }
...
function handleMessageListClick(event) {
    ...
    if (mediaTrigger) {
        const mediaGroup = mediaTrigger.dataset.mediaGroup
            ? mediaTrigger.dataset.mediaGroup.split(",").filter(Boolean)
            : [mediaTrigger.dataset.openMedia];
        openMediaModal(mediaTrigger.dataset.openMedia, mediaGroup);
    }
}

function openMediaModal(mediaId, mediaGroup = [mediaId]) {
    const media = state.mediaStore.get(mediaId);
    const modal = $("media-modal");
    if (!modal || !media) return;

    state.activeMediaId = mediaId;
    state.activeMediaGroup = mediaGroup.filter((id) => state.mediaStore.has(id));
    if (!state.activeMediaGroup.length) {
        state.activeMediaGroup = [mediaId];
    }
    state.activeMediaIndex = Math.max(0, state.activeMediaGroup.indexOf(mediaId));

    renderActiveMediaModal();

    modal.hidden = false;
    requestAnimationFrame(() => modal.classList.add("open"));
}

function renderActiveMediaModal() {
    const mediaId = state.activeMediaGroup[state.activeMediaIndex] || state.activeMediaId;
    const media = state.mediaStore.get(mediaId);
    ...
    if (!body || !subtitle || !downloadLink || !title || !media) return;

    state.activeMediaId = media.id;
    title.innerText = state.activeMediaGroup.length > 1 ? "Media Gallery" : "Media Preview";
    subtitle.innerText = state.activeMediaGroup.length > 1
        ? `${state.activeMediaIndex + 1} / ${state.activeMediaGroup.length} • ${media.name}`
        : `${labelForMediaKind(media.kind)} • ${media.name}`;
    downloadLink.href = media.url;
    downloadLink.download = media.name;

    const viewer = renderModalViewer(media);
    const nav = state.activeMediaGroup.length > 1 ? `
        <button class="media-nav prev" type="button" data-media-nav="-1" aria-label="Previous media"><i class="ph ph-caret-left"></i></button>
        <button class="media-nav next" type="button" data-media-nav="1" aria-label="Next media"><i class="ph ph-caret-right"></i></button>
    ` : "";
    const strip = state.activeMediaGroup.length > 1 ? renderMediaThumbnailStrip() : "";
...
function renderModalViewer(media) {
    if (media.kind === "image" || media.kind === "sticker") {
        return `<img src="${escapeAttribute(media.url)}" alt="${escapeAttribute(media.name)}" decoding="async">`;
    }
    if (media.kind === "video") {
        return `<video controls autoplay src="${escapeAttribute(media.url)}"></video>`;
    }
    if (media.kind === "audio") {
        return `<audio controls autoplay src="${escapeAttribute(media.url)}"></audio>`;
    }
    return `
        <div class="media-doc">
            <div class="media-doc-head">
...
function renderMediaThumbnailStrip() {
    return `
        <div class="media-thumb-strip">
            ${state.activeMediaGroup.map((id, index) => {
                const media = state.mediaStore.get(id);
                if (!media) return "";
                const activeClass = index === state.activeMediaIndex ? "active" : "";
                if (media.kind === "image" || media.kind === "sticker") {
                    return `<button type="button" class="media-thumb ${activeClass}" data-media-index="${index}"><img src="${escapeAttribute(media.url)}" alt="${escapeAttribute(media.name)}"></button>`;
                }
                if (media.kind === "video") {
                    return `<button type="button" class="media-thumb ${activeClass}" data-media-index="${index}"><span class="media-thumb-icon"><i class="ph-fill ph-play"></i></span></button>`;
                }
                return `<button type="button" class="media-thumb ${activeClass}" data-media-index="${index}"><span class="media-thumb-icon"><i class="ph-fill ph-file"></i></span></button>`;
            }).join("")}
        </div>
    `;
}
...
function handleMediaModalClick(event) {
    ...
    if (navButton) {
        const direction = Number(navButton.dataset.mediaNav || "0");
        stepActiveMedia(direction);
        return;
    }

    const thumbButton = event.target.closest("[data-media-index]");
    if (thumbButton) {
        state.activeMediaIndex = Number(thumbButton.dataset.mediaIndex || "0");
        renderActiveMediaModal();
    }
}

function stepActiveMedia(direction) {
    if (!state.activeMediaGroup.length) return;
    const total = state.activeMediaGroup.length;
    state.activeMediaIndex = (state.activeMediaIndex + direction + total) % total;
    renderActiveMediaModal();
}
...
async function printChatAsPdf() {
    ...
    closeMenu();
    prepareFullRenderForPrint();
    await ensurePrintMediaLoaded();
    window.setTimeout(() => window.print(), 80);
}

function prepareFullRenderForPrint() {
    ...
    if (state.isSearchOpen) {
        $("search-toolbar")?.classList.remove("active");
...
function handleAfterPrint() {
    ...
    clearPrintVideoPosters();
    state.renderRange = state.printSnapshot.renderRange;
    state.isSearchOpen = state.printSnapshot.isSearchOpen;
    $("search-toolbar")?.classList.toggle("active", state.isSearchOpen);
    state.printSnapshot = null;
    renderChatList();
}

async function ensurePrintMediaLoaded() {
    const viewport = $("viewport");
    if (!viewport) return;

    const lazyElements = [...viewport.querySelectorAll("img[data-src], video[data-src]")];
    lazyElements.forEach(loadLazyMediaElement);

    const mediaElements = [...viewport.querySelectorAll(".message-list img, .message-list video")];
    await Promise.all(mediaElements.map((element) => waitForPrintableMedia(element)));
    await buildPrintVideoPosters(viewport);
}

function waitForPrintableMedia(element) {
...
async function buildPrintVideoPosters(root) {
    clearPrintVideoPosters();

    const videos = [...root.querySelectorAll(".message-list video")];
    for (const video of videos) {
        ...
        image.src = poster;
        image.alt = video.getAttribute("aria-label") || "Video preview";
        image.className = "print-video-poster";
        video.classList.add("print-has-poster");
        video.insertAdjacentElement("afterend", image);
        state.printPreviewNodes.push(image);
    }
}

function clearPrintVideoPosters() {
    document.querySelectorAll(".print-has-poster").forEach((node) => node.classList.remove("print-has-poster"));
    state.printPreviewNodes.forEach((node) => node.remove());
    state.printPreviewNodes = [];
}

async function captureVideoPoster(video) {
    try {
        if (video.readyState < 2) {
            await waitForPrintableMedia(video);
        }
        ...
        const targetTime = video.duration && Number.isFinite(video.duration)
            ...
            : 0;
        await seekVideoForPoster(video, targetTime);
        ...
        await seekVideoForPoster(video, originalTime);
        return canvas.toDataURL("image/jpeg", 0.92);
    } catch (error) {
        console.error("Could not capture video poster for print", error);
        return "";
    }
}

function seekVideoForPoster(video, time) {
    return new Promise((resolve) => {
...
function handleGlobalKeydown(event) {
    ...
    if (state.activeMediaId && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
        event.preventDefault();
        stepActiveMedia(event.key === "ArrowLeft" ? -1 : 1);
    }
}
...
async function copyUPI() {
    try {
        await navigator.clipboard.writeText("zhangjian8803@gmail.com");
...
function hasMatchedOwnSender() {
    if (!state.myName) return false;
    return Object.keys(state.senderStats).some((sender) => isMeSender(sender));
}
